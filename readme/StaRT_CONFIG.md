...
## 12. 其他内核控制开关 (新增)
### START_USING_UDELAY
- 1: 开启无内核态阻塞的 `s_udelay()` 高精度纯循环微秒级延时函数。
- 0: 裁剪相关 API，节省代码空间。

### START_IDLE_HOOK_LIST_SIZE
- 配置允许在 Idle 线程中注册的钩子函数最大数量（支持数组遍历以实现多个钩子）。
- 例如配置为 `4` 时，支持最高 4 个单独的钩子；如果配置为 `0` 则完全关闭 Idle 钩子功能，恢复原版纯净 WFI 模式逻辑。