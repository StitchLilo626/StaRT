...
;                                 全局变量
;*************************************************************************   
	IMPORT rtos_prev_thread_sp_p
    IMPORT rtos_next_thread_sp_p
	IMPORT rtos_interrupt_flag
;*************************************************************************
;                                 常量
;*************************************************************************
;-------------------------------------------------------------------------
...
; */
rtos_irq_disable    PROC
    EXPORT  rtos_irq_disable
    MRS     r0, PRIMASK
    CPSID   I
    BX      LR
    ENDP
...
; */
rtos_irq_enable    PROC
    EXPORT  rtos_irq_enable
    MSR     PRIMASK, r0
    BX      LR
    ENDP
...
;/*
; *-----------------------------------------------------------------------
; * 函数原型：void rtos_frist_switch_task(rt_uint32 to);
; * r0 --> to
; * 该函数用于开启第一次线程切换
; *-----------------------------------------------------------------------
; */
		
rtos_frist_switch_task    PROC
    
	; 导出rt_hw_context_switch_to，让其具有全局属性，可以在C文件调用
	EXPORT rtos_frist_switch_task
		
    ; 设置rt_interrupt_to_thread的值
    LDR     r1, =rtos_next_thread_sp_p             ;将rt_interrupt_to_thread的地址加载到r1
    STR     r0, [r1]                                ;将r0的值存储到rt_interrupt_to_thread

    ; 设置rt_interrupt_from_thread的值为0，表示启动第一次线程切换
    LDR     r1, =rtos_prev_thread_sp_p           ;将rt_interrupt_from_thread的地址加载到r1
    MOV     r0, #0x0                                ;配置r0等于0
    STR     r0, [r1]                                ;将r0的值存储到rt_interrupt_from_thread

    ; 设置中断标志位rt_thread_switch_interrupt_flag的值为1
    LDR     r1, =rtos_interrupt_flag    ;将rt_thread_switch_interrupt_flag的地址加载到r1
    MOV     r0, #1                                  ;配置r0等于1
...
;/*
; *-----------------------------------------------------------------------
; * void rtos_normal_switch_task(rt_uint32 from, rt_uint32 to);
; * r0 --> from
; * r1 --> to
; *-----------------------------------------------------------------------
; */
...
rtos_normal_switch_task    PROC
    EXPORT rtos_normal_switch_task

    ; 设置中断标志位rt_thread_switch_interrupt_flag为1     
    LDR     r2, =rtos_interrupt_flag                      ; 加载rt_thread_switch_interrupt_flag的地址到r2
	...
	; rtos_prev_thread_sp_p
    LDR     r2, =rtos_prev_thread_sp_p                    ; 加载rt_interrupt_from_thread的地址到r2
    STR     r0, [r2]                                      ; 存储r0的值到rt_interrupt_from_thread，即上一个线程栈指针sp的指针

_reswitch
    ; 设置rt_interrupt_to_thread的值
	LDR     r2, =rtos_next_thread_sp_p                   ; 加载rt_interrupt_from_thread的地址到r2
...
PendSV_Handler   PROC
    ...
    ; 获取中断标志位，看看是否为0
    LDR     r0, =rtos_interrupt_flag     		     ; 加载rt_thread_switch_interrupt_flag的地址到r0
    ...
    LDR     r0, =rtos_prev_thread_sp_p            	 ; 加载rt_interrupt_from_thread的地址到r0
...
; ========================== 下文切换 ==============================
switch_to_thread
    LDR     r1, =rtos_next_thread_sp_p               ; 加载rt_interrupt_to_thread的地址到r1
    ...
    LDMFD   r1!, {r4 - r11}                           ;将线程栈指针r1(操作之前先递增)指向的内容加载到CPU寄存器r4~r11
